#!/usr/bin/env bash
# 构建脚本：将 C 源码编译为 WebAssembly + 胶水 JS（由 Emscripten 自动生成）。
...
FONT_SRC="${CHESS_FONT_PATH:-/c/Windows/Fonts/msyh.ttc}"

if [ ! -f "$FONT_SRC" ]; then
  echo "找不到字体文件：$FONT_SRC"
  echo "请通过 CHESS_FONT_PATH 环境变量指定一个支持中文的 .ttf/.ttc 字体路径。"
  exit 1
fi
...
emcc \
  ...
  -sASSERTIONS=1 \
  --preload-file "${FONT_SRC}@/fonts/msyh.ttc" \
  --shell-file shell.html \
  -O2 \
  -o dist/index.html

echo ""
echo "构建完成：dist/index.html"
echo "本地预览：cd dist && python -m http.server 8000  然后打开 http://localhost:8000"
